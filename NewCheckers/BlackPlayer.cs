...
namespace NewCheckers
{
    ...
    {
        public static void BlackStepFrom()
        {
            ...
            ConsoleKey move = 0;
            byte coun = 0;
            if (!BlackTestKill() && !DamTestKill(2))
            {
                while (1 == 1)
                {
        ...
        private static void BlackStepIn(int[] arr)
        {
            int k = arr[0];
            int t = arr[1];
            int status = 0;
        ...
        public static bool BlackTestKill()
        {
            bool result = false;
            byte count = 0;
                        ...
                        try
                        {
                            if (kill[i - 1, j - 1] >= 1)
                            {
                                if (Field.field[i - 2, j - 2] && kill[i - 2, j - 2] == 0)
                                {
                                    result = true;
                                    count++;
                                    //Console.WriteLine("Вы можете срубить синюю шашку из " + (j + 1) + " , " + (i + 1) + " в " + (j - 1) + " , " + (i - 1));
                                    killFrom[count - 1, 0] = j + 1;
                                    killFrom[count - 1, 1] = i + 1;
                                    killIn[count - 1, 0] = j - 1;
                                    killIn[count - 1, 1] = i - 1;
                                }
                            }
                        }
                        catch (IndexOutOfRangeException) { }
                        try
                        {
                            if (kill[i - 1, j + 1] >= 1)
                            {
                                if (Field.field[i - 2, j + 2] && kill[i - 2, j + 2] == 0)
                                {
                                    result = true;
                                    count++;
                                    //Console.WriteLine("Вы можете срубить синюю шашку из " + (j + 1) + " , " + (i + 1) + " в " + (j + 3) + " , " + (i - 1));
                                    killFrom[count - 1, 0] = j + 1;
                                    killFrom[count - 1, 1] = i + 1;
